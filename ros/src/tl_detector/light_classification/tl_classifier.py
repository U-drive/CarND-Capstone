...
class TLClassifier(object):
    def __init__(self):
	PATH_TO_MODEL = 'models/frozen_inference_graph.pb' # model resnet-udacity-sim-large-10-regions
	...
	#rospy.loginfo('Classe: {}'.format(classes))
        ...
        if self.save_images == True and self.saved_image_counter <= self.saved_image_limit:
            if not (os.path.exists("./tl_images_inference")):
                os.mkdir("./tl_images_inference")
            cv2.imwrite("./tl_images_inference/inference_image{0:0>4}.jpeg".format(self.saved_image_counter),cv2.cvtColor(image,cv2.COLOR_RGB2BGR))
	    rospy.loginfo('Image : inference_image{0:0>4}.jpeg'.format(self.saved_image_counter))
            self.saved_image_counter += 1

 	#rospy.loginfo('Score: {}'.format(scores[0]))
        if scores[0] > CLASSIFICATION_THRESHOLD: 
